...
const forbidden = [/Enterprise UI/g, /Axuran/g, /Lattice UI/g, /KendoReact clone/gi, /Kendo clone/gi];
for (const p of customerFacing) {
  const text = read(p);
  for (const rx of forbidden) if (rx.test(text)) fail(`${p}: forbidden customer-facing legacy branding ${rx}`);
}

...
if (!agents.includes('strictly local-only')) fail('AGENTS.md missing strict local-only reference rule');
if (!policy.includes('Online fallback is disabled')) fail('reference policy does not disable online fallback');
if (!prompt.includes('Mandatory local reference preflight')) fail('component-stage prompt missing local preflight');

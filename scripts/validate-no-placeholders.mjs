...
const governed = [
  'AGENTS.md','ARCHITECTURE.md','PARITY_DEFINITION.md','DEPENDENCY_POLICY.md','KENDO_REFERENCE_POLICY.md',
  ...files('skills').filter((p) => p.endsWith('.md')),
  ...files('.agent/workflows').filter((p) => p.endsWith('.md')),
];
const bad = [/\blorem ipsum\b/i,/\bTBD\b/,/\bFIXME\b/,/placeholder content/i,/enterprise implementation skill\.?$/im];

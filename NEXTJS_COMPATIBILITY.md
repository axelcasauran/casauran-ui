...
`next start` for the visual-test and documentation hosts. The infrastructure probe checks server-rendered markup and a
...
The F0.18 documentation shell, route content, table of contents, examples, API/accessibility
structures, and registry-derived metadata are server-rendered by default. One local client module
owns enumerated theme/density/direction changes after hydration; the server and first client render
use identical explicit root attributes. No stage-registry payload is shipped to that boundary.
